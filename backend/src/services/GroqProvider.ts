...
export class GroqProvider implements AIProvider {
  ...
  async analyze(industry: string, searchResults: SearchResult[], modelId?: string): Promise<AnalysisResponse> {
    if (!this.apiKey) {
      console.warn('GROQ_API_KEY is not defined. Returning high-quality mock analysis.');
      return this.getMockAnalysis(industry);
    }

    let model = this.defaultModel;
    if (modelId) {
      if (modelId.includes('llama') || modelId.includes('deepseek')) {
        // deepseek-r1-distill-llama-70b has been decommissioned on Groq, fallback to llama-3.3-70b-versatile
    ...
    console.log(`Analyzing industry "${industry}" using Groq model "${model}"...`);
...
3. A Startup Idea to address the problem, including:
   ...
   - Feature 6 Blueprint fields:
     ...
     * Database Recommendation (even if the current app is database-free, recommend what this startup would use, e.g., PostgreSQL, Supabase, Pinecone)
...
`;
    ...
    try {
      ...
      // Extract JSON if model returned thinking tags
      if (content.includes('<think>')) {
        content = content.replace(/<think>[\s\S]*?<\/think>/g, '');
      }

      // Safeguard: try finding JSON boundaries if there is leading/trailing text
  ...
  private getMockAnalysis(industry: string): AnalysisResponse {
    // Shared mock analysis implementation
      ...
      startupIdeas: [
        {
          id: 'idea-1',
          problemId: 'prob-1',
          name: `${industry.replace(/\s+/g, '')}Flow (Groq)`,
