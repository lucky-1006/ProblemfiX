...
export class GroqProvider implements AIProvider {
  ...
  async analyze(industry: string, searchResults: SearchResult[], modelId?: string): Promise<AnalysisResponse> {
    ...
    if (modelId) {
      ...
      } else if (modelId.includes('llama') || modelId.includes('deepseek')) {
    ...
    const llmCaller = async (systemPrompt: string, userPrompt: string): Promise<string> => {
      ...
      for (const currentModel of modelsToTry) {
        try {
          console.log(`[GroqProvider] Attempting API call with model: "${currentModel}"`);
          ...
          console.warn(`[GroqProvider] Model "${currentModel}" failed (Status: ${status}): ${errMsg}`);
