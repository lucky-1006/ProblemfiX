...
export class OpenAIProvider implements AIProvider {
  ...
  constructor() {
    ...
    this.defaultModel = 'gpt-4o'; // Primary model mapping for GPT-5.5
  }

  async analyze(industry: string, searchResults: SearchResult[], modelId?: string): Promise<AnalysisResponse> {
    if (!this.apiKey) {
      console.warn('OPENAI_API_KEY is not defined. Returning high-quality mock analysis.');
      return this.getMockAnalysis(industry);
    }

    const model = modelId || this.defaultModel;
    console.log(`Analyzing industry "${industry}" using OpenAI model "${model}"...`);
...
`;
    ...
    try {
      const response = await axios.post('https://api.openai.com/v1/chat/completions', {
        model: model === 'gpt-5.5' ? 'gpt-4o' : model, // Map UI label gpt-5.5 to gpt-4o
        messages: [
      ...
      throw new Error(`OpenAI API failed: ${errMsg}`);
    }
  }
  ...
  private getMockAnalysis(industry: string): AnalysisResponse {
    ...
    // Default mock data for any other industry
    return {
      problems: [
        {
          id: 'prob-1',
      ...
      startupIdeas: [
        {
          id: 'idea-1',
          problemId: 'prob-1',
          name: `${industry.replace(/\s+/g, '')}Flow`,
